...
func SplitWhiteSpaces(s string) []string {
	...
	for i := 0; i < len(s); i++ {
		...
		if rune(s[i]) == ' ' { // si la string s de variable s est strictement égale à un espace
			printstr = append(printstr, s[j:i]) // dans le tableau printstr, append va ranger ce que contient s en séparant les mots                           // i + 1 car avec +2 on supprime la première lettre de tout les mots qui suis le premier et un - 1 ne peux pas être possible car sinon ça affiche la dernière lettre du mot précédent en plus du second mot
